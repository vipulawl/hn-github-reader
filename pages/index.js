...
function DetailPane({ item, comments, commentsLoading }) {
  if (!item) {
    return (
      ...
      <div className="max-w-4xl p-6 mx-auto">
          ...
          {github?.topics?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {github.topics.slice(0, 12).map(t => (
                <span
                  key={t}
                  className="text-xs px-2 py-0.5 bg-blue-950/60 text-blue-400 rounded-full border border-blue-900/50"
                >
                  {t}
                </span>
              ))}
        ...
        {github && (
          <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 pb-5 mb-5 border-b border-gray-800/60">
            <LangDot language={github.language} />
            {github.license?.spdx_id && github.license.spdx_id !== 'NOASSERTION' && (
              <span>📄 {github.license.spdx_id}</span>
            )}
            {github.pushed_at && (
              <span>Last push {timeAgo(github.pushed_at)}</span>
            )}
            {github.created_at && (
              <span>Created {new Date(github.created_at).getFullYear()}</span>
            )}
            {github.homepage && (
              <a
...
export default function Home() {
  ...
  return (
    <>
      <Head>
        <title>HN × GitHub — Trending Repos</title>
        <meta name="description" content="Trending GitHub repos surfaced from Hacker News" />
        ...
        <header className="border-b border-gray-800/80 px-5 py-3 flex items-center justify-between flex-shrink-0 bg-[#0d1117]/95 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-600 hidden sm:block border-l border-gray-800 pl-3">
              Trending repos from Hacker News
            </span>
          </div>
          <div className="flex items-center gap-3">
            {fetchedAt && !loading && (
              ...
              {loading && items.length === 0
                ...
                : items.map(item => (
                    <StoryCard
                      key={item.hnId}
                      item={item}
                      isSelected={selected?.hnId === item.hnId}
                      onClick={setSelected}
                    />
                  ))
              }
